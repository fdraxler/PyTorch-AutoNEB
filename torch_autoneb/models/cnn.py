...
class CNN(Module):
    def __init__(self, conv_depth, num_filters, padding_size, pool_size, pool_every, filter_size, dense_depth, num_dense_units, input_size, output_size, batch_norm=True, dropout=0):
        super().__init__()

        # Need to keep track of dimensions to know which linear layer we need later
        in_channels = int(input_size[0])
        dims = self.input_size[1:]
        ...
        for i in range(conv_depth):
            ...
            layer["conv" % i] = Conv2d(in_channels, num_filters[i], kernel_size=filter_size, padding=padding_size)
            ...
            conv_layers[f"layer_{i}"] = (Sequential(layer))
        ...
        for i in range(dense_depth):
            ...
            dense_layers[f"layer_{i}"] = Sequential(layer)
